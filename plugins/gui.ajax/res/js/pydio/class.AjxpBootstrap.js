...
Class.create("AjxpBootstrap", {
	...
	insertLoaderProgress : function(targetContainer, passedParameters){
		...
		if(this.parameters.get('customWelcomeScreen')){
            ...
            if(customWording.title.toLowerCase() != "ajaxplorer"){
				html+='	<div style="padding:4px 7px;position: relative;"><div>Sistema de Gestión de Calidad<span id="version_span"></span></div>';
			}else{
	...
	insertBasicSkeleton : function(desktopNode){
        ...
        if(document.viewport.getWidth() < 500){
            elem.update('SGC - UTSMA &copy; C. du Jeu 2008-2013');
        }else{
            elem.update('SGC, Sistema de Gestión de Calidad - <a target="_blank" style="color: #7a7a7a;" href="http://utsma.edu.mx/">http://utsma.edu.mx/</a>');
        }
